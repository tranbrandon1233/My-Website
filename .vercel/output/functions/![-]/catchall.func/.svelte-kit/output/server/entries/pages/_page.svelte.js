...
function Main($$renderer) {
	...
	Step($$renderer, {
		step: steps[1],
		children: ($$renderer) => {
			$$renderer.push(`<p>A virtual reality physical therapy system that uses a Gemini LLM assistant to guide users through physical therapy exercises created with <strong class="text-violet-400">Python, OpenAI, and PyTorch</strong> that allows users to select exercises and track their progress.</p>`);
		},
		$$slots: { default: true }
	});
	$$renderer.push(`<!----> `);
	Step($$renderer, {
		step: steps[2],
		children: ($$renderer) => {
			$$renderer.push(`<p>A hands-free phone that can answer any question powered by <strong class="text-violet-400">GPT-4, LangChain, and APIs from SerpAPI, Yelp, Twilio</strong>, and more!</p>`);
		},
		$$slots: { default: true }
	});
	$$renderer.push(`<!----> `);
	...
	Step($$renderer, {
		step: steps[4],
		children: ($$renderer) => {
			$$renderer.push(`<p>A mobile app created with <strong class="text-violet-400">Flutter and Firebase</strong> that allows users to quickly and discreetly alert trusted contacts and/or emergency services if they are in danger.</p>`);
		},
		$$slots: { default: true }
	});
	$$renderer.push(`<!----> `);
	...
	Step($$renderer, {
		step: steps[6],
		children: ($$renderer) => {
			$$renderer.push(`<p>A Natural Language Processing (NLP) model that detects tone in text using a dataset of positive and negative IMDB movie reviews using <strong class="text-violet-400">PyTorch, Pandas, and Scikit-learn</strong>.</p>`);
		},
		$$slots: { default: true }
	});
	$$renderer.push(`<!----> `);
	Step($$renderer, {
		step: steps[7],
		children: ($$renderer) => {
			$$renderer.push(`<p>The official dashboard website for Fixated that allows users to view and analyze data from their Youtube and X/Twitter accounts using <strong class="text-violet-400">React, TailwindCSS, and Vite</strong> for a dashboard that displays data in a visually appealing and easy to understand way.</p>`);
		},
		$$slots: { default: true }
	});
	$$renderer.push(`<!----> `);
	Step($$renderer, {
		step: steps[8],
		children: ($$renderer) => {
			$$renderer.push(`<p>End-to-end clothes customization platform with built-in AI try-on options. Built with <strong class="text-violet-400">TypeScript, TailwindCSS, React, Express, and AWS RDS, S3, Lambda, Amplify, Route 53, and more</strong>.</p>`);
		},
		$$slots: { default: true }
	});
