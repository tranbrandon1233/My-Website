...
function _layout($$renderer, $$props) {
	let y;
	let innerHeight = 0;
	$$renderer.push(`<div class="container relative flex flex-col max-w-[1400px] mx-auto w-full text-sm sm:text-base min-h-screen overflow-x-scroll [scrollbar-width:none] [&amp;::-webkit-scrollbar]:hidden"><div${attr_class("fixed bottom-0 w-full duration-200 flex p-10 z-[10] " + (y > 0 ? " opacity-full pointer-events-auto" : " pointer-events-none opacity-0"))}><button class="ml-auto rounded-full bg-slate-900 text-violet-400 px-3 sm:px-4 hover:bg-slate-800 cursor-pointer aspect-square grid place-items-center"><i class="fa-solid fa-arrow-up"></i></button></div> `);
	Header($$renderer, {
		y,
		innerHeight
	});
